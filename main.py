...
# ----------------------------
# Analysis endpoint using coordinator agent
# ----------------------------
@app.post("/entries/{entry_id}/analyze")
def analysis_endpoint(entry_id: int, session: Session = Depends(get_session)):
    entry = session.get(Entry, entry_id)
    if not entry:
        raise HTTPException(status_code=404, detail="Entry not found")

    # Prepare the state dictionary for coordinator
    state = {
        "messages": [],
        "gratitude": entry.gratitude,
        "tasks": entry.tasks,
